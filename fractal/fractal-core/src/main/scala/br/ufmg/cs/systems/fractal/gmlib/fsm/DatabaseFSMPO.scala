...
class DatabaseFSMPO(minSupport: Double, numGraphs: Int, maxNumEdges: Int,
   ...
   @transient lazy private val key: EdgeInducedSubgraph => Pattern = s => s
      .quickPattern()

   // reusable support value
   @transient lazy private val value: EdgeInducedSubgraph => HashSet[Int] = s
   => {
      HashSet(vertexToGraphIdxBc.value(s.getVertices.getLast))
   }

   // support aggregation function
   @transient lazy private val aggregate: (HashSet[Int],HashSet[Int]) => Unit =
      (supp1, supp2) => {
