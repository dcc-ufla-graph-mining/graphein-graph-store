...
def _validate_graph_reconstruction(extracted_graph, original_graph, pdb_code):
    ...
    try:
        ...
        for (u, v, attr) in extracted_graph.edges[i].data():
            print(u, v, attr)
            for k, t in attr.items():
                print(type(t))
            break
        for (u, v, attr) in original_graph[i].edges.data():  
            print(u, v, attr) 
            for k, t in attr.items():
                print(type(t))
            break
...
def _reconstruct_and_validate_graphs(protein_graphs, node_to_id, edge_to_id, 
    ...
    for pdb_code in protein_graphs:
        ...
        print("type(edge_construction_functions)", type(funcs))
        granularity = "N"  
        params = {
            "granularity": granularity,
            "edge_construction_functions": funcs,
        }
        config = ProteinGraphConfig(**params)

        print(config.model_dump())
        print(f"Reconstructing graph for {pdb_code} with config: {config.model_dump()}")
        print(f"{pdb_dir}/{pdb_code.lower()}.pdb")
        print(funcs)
        if not os.path.exists(f"{pdb_dir}/{pdb_code.lower()}.pdb"):
            print(f"File {pdb_dir}/{pdb_code.lower()}.pdb does not exist. Skipping reconstruction.")
        
        original_graph = construct_graph(config=config, path=f"{pdb_dir}/{pdb_code.lower()}.pdb")
        ...
        i = 0
        for u, v in extracted_graph.edges():
            if (u, v) not in edges:
                edges.append((u, v))
        ...
        print("extracted graph edges size", len(extracted_graph.edges()))
        print("original graph edges size", len(original_graph.edges()))
            # erro grave: extracted ta pegando arestas que nao existem no original

        for edge in edges:
            print("extracted graph edge", extracted_graph.edges[edge])
            print("original graph edge", original_graph.edges[edge])
            

        for node in nodes:
            print("original graph node", original_graph.nodes[node])
            print("extracted graph node", extracted_graph.nodes[node])
