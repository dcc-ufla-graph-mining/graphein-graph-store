...
def main():
    ...
    for i in range(BUILDER_REPLICAS):
        builder_queue.put(Builder.remote(i))

    for i in range(PARSER_REPLICAS):
        parser_queue.put(Parser.remote(i))

    for i in range(EXECUTOR_REPLICAS):
...
#ppi graph build test
'''
b = Builder.remote("ppi", ['AP3B1', 'BRD4', 'BRD2', 'CWC27', 'ZC3H18', 'SLC44A2', 'PMPCB',
...
graph = ray.get(b.get_graph.remote())
